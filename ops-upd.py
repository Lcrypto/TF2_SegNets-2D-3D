...
def batch_norm(x, name="batch_norm"):
    return tf.contrib.layers.batch_norm(x, decay=0.9, updates_collections=tf.compat.v1.GraphKeys.UPDATE_OPS,
                                        epsilon=1e-5, scale=True, scope=name)
...
def apply_bicubic_downsample(x, filter, factor):
    ...
    x = tf.nn.depthwise_conv2d(input=x, filter=filter, strides=[1, strides, strides, 1], padding='VALID')
    # x = x/127.5 - 1
    return x
