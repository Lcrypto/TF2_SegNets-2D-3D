...
def generator_resnetYDW(image, options, reuse=False, name="generator",
                        g3Flag=False):  # this has been modified to suit c2gan and acgan for poisson noise removal and downsampling
...
def edsrYDW(image, options, reuse=False, name='EDSRYDW', numResBlocks=16):
    with tf.variable_scope(name):
        ...
        c0 = tf.nn.relu(c0)  # where should the activation go?
        # do the wdsr skip
        wideOut = tf.nn.relu(tf.nn.depth_to_space(conv2d(shallow, options.output_c_dim * 16, 5, 1, name='wideSkip'), 4,
                                                  name='wideSubPix'))
...
def discriminator(image, options, reuse=False, name="discriminator",
                  d2Flag=False):  # patchGAN doesnt work well with minibatches
...
def discriminatorSR(image, options, reuse=False, name="discriminator",
                    d2Flag=False):  # the SRGAN discriminator modified to fit acgan
    with tf.variable_scope(name):
        if reuse:
            tf.get_variable_scope().reuse_variables()
        # else:
...
def segNetYDW(image, options, reuse=False, name='segNetYDW', numResBlocks=16):  # the original segnet
    with tf.variable_scope(name):
        ...
        x1 = tf.nn.max_pool(
            tf.nn.dropout(tf.nn.relu(batchnormSR(conv2d(x, options.gf_dim, 7, 1, padding='SAME', name='g_e1_c'))),
                          dropout_rate), ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool1')
        x2 = tf.nn.max_pool(
            tf.nn.dropout(tf.nn.relu(batchnormSR(conv2d(x1, options.gf_dim * 2, 7, 1, padding='SAME', name='g_e2_c'))),
                          dropout_rate), ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool2')
        x3 = tf.nn.max_pool(
            tf.nn.dropout(tf.nn.relu(batchnormSR(conv2d(x2, options.gf_dim * 4, 7, 1, padding='SAME', name='g_e3_c'))),
                          dropout_rate), ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool3')
        x4 = tf.nn.max_pool(
            tf.nn.dropout(tf.nn.relu(batchnormSR(conv2d(x3, options.gf_dim * 8, 7, 1, padding='SAME', name='g_e4_c'))),
                          dropout_rate), ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool4')

        x = deconv2d(x4, options.gf_dim, 2, 2, name='g_e5_dc')
        x = tf.nn.dropout(tf.nn.relu(batchnormSR(conv2d(x, options.gf_dim * 8, 7, 1, padding='SAME', name='g_e5_c'))),
                          dropout_rate)

        x = deconv2d(x, options.gf_dim, 2, 2, name='g_e6_dc')
        x = tf.nn.dropout(tf.nn.relu(batchnormSR(conv2d(x, options.gf_dim * 4, 7, 1, padding='SAME', name='g_e6_c'))),
                          dropout_rate)

        x = deconv2d(x, options.gf_dim, 2, 2, name='g_e7_dc')
        x = tf.nn.dropout(tf.nn.relu(batchnormSR(conv2d(x, options.gf_dim * 2, 7, 1, padding='SAME', name='g_e7_c'))),
                          dropout_rate)

        x = deconv2d(x, options.gf_dim, 2, 2, name='g_e8_dc')
        x = tf.nn.dropout(tf.nn.relu(batchnormSR(conv2d(x, options.gf_dim, 7, 1, padding='SAME', name='g_e8_c'))),
                          dropout_rate)
...
def segNetYDW4(image, options, reuse=False, name="segNetYDW", numResBlocks=16):  # a Res-Unet-segnet with k=4
    ...
    with tf.variable_scope(name):
        ...
        def residualBlock(x, dim, ks=4, s=1, name='resBlock', dropout_rate=0.5, resSkip=False):
            ...
            if resSkip:
                y = tf.nn.dropout(tf.nn.relu(batchnormSR(conv2d(y, dim, ks, s, padding='SAME', name=name + '_c2'))),
                                  dropout_rate)
                return y + x
            else:
        ...
        x2 = residualBlock(tf.nn.max_pool(x1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool1'),
                           options.gf_dim * 2, ks=4, s=1, name='resBlock2', dropout_rate=dropout_rate, resSkip=resSkip)

        x3 = residualBlock(tf.nn.max_pool(x2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool2'),
                           options.gf_dim * 4, ks=4, s=1, name='resBlock3', dropout_rate=dropout_rate, resSkip=resSkip)

        x4 = residualBlock(tf.nn.max_pool(x3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool3'),
                           options.gf_dim * 8, ks=4, s=1, name='resBlock4', dropout_rate=dropout_rate, resSkip=resSkip)

        # trough conv conv
        x5 = residualBlock(tf.nn.max_pool(x4, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool4'),
                           options.gf_dim * 16, ks=4, s=1, name='resBlockT', dropout_rate=dropout_rate, resSkip=resSkip)

        # updeconv, concat, conv, conv
        y1 = batchnormSR(deconv2d(x5, options.gf_dim * 8, ks=4, name='g_d4'))
        if Uskip:
            y1 = tf.concat([y1, x4], 3)
        y1 = residualBlock(y1, options.gf_dim * 8, ks=4, s=1, name='resBlock5', dropout_rate=dropout_rate,
                           resSkip=resSkip)

        y2 = batchnormSR(deconv2d(y1, options.gf_dim * 4, ks=4, name='g_d5'))
        if Uskip:
            y2 = tf.concat([y2, x3], 3)
        y2 = residualBlock(y2, options.gf_dim * 4, ks=4, s=1, name='resBlock6', dropout_rate=dropout_rate,
                           resSkip=resSkip)

        y3 = batchnormSR(deconv2d(y2, options.gf_dim * 2, ks=4, name='g_d6'))
        if Uskip:
            y3 = tf.concat([y3, x2], 3)
        y3 = residualBlock(y3, options.gf_dim * 2, ks=4, s=1, name='resBlock7', dropout_rate=dropout_rate,
                           resSkip=resSkip)
...
def segNetYDW3D(image, options, reuse=False, name="segNetYDW3D", numResBlocks=16):  # a Res-Unet-segnet with k=4
    ...
    with tf.variable_scope(name):
        ...
        def residualBlock(x, dim, ks=4, s=1, name='resBlock', dropout_rate=0.5, resSkip=False):
            ...
            if resSkip:
                y = tf.nn.dropout(tf.nn.relu(batchnormSR(conv3d(y, dim, ks, s, padding='SAME', name=name + '_c2'))),
                                  dropout_rate)
                return y + x
            else:
                return tf.nn.dropout(y, dropout_rate)
        ...
        x2 = residualBlock(
            tf.nn.max_pool3d(x1, ksize=[1, 2, 2, 2, 1], strides=[1, 2, 2, 2, 1], padding='SAME', name='pool1'),
            options.gf_dim * 2, ks=4, s=1, name='resBlock2', dropout_rate=dropout_rate, resSkip=resSkip)

        x3 = residualBlock(
            tf.nn.max_pool3d(x2, ksize=[1, 2, 2, 2, 1], strides=[1, 2, 2, 2, 1], padding='SAME', name='pool2'),
            options.gf_dim * 4, ks=4, s=1, name='resBlock3', dropout_rate=dropout_rate, resSkip=resSkip)

        x4 = residualBlock(
            tf.nn.max_pool3d(x3, ksize=[1, 2, 2, 2, 1], strides=[1, 2, 2, 2, 1], padding='SAME', name='pool3'),
            options.gf_dim * 8, ks=4, s=1, name='resBlock4', dropout_rate=dropout_rate, resSkip=resSkip)

        # trough conv conv
        x5 = residualBlock(
            tf.nn.max_pool3d(x4, ksize=[1, 2, 2, 2, 1], strides=[1, 2, 2, 2, 1], padding='SAME', name='pool4'),
            options.gf_dim * 16, ks=4, s=1, name='resBlockT', dropout_rate=dropout_rate, resSkip=resSkip)
...
def tot_var_criterion(
        image):  # this is just the image gradient loss. it must be combined with the abs or mae criterion for balance
...
def seg_criterion(logits, labels):
    ...
    labels = tf.reshape(tf.one_hot(labels, logits.shape[nDims - 1]),
                        [-1, logits.shape[nDims - 1]])  # flatten in batch and space dims
